...
def get_react(dataset, gene_names):
	...
	for name in gene_names:
		print "the current gene name in a list of genes in a reaction ",name
		if name in dataset:
			print "the name is in the dataset"
			for i in range(0, len(dataset[name])):
				print "this is being added to r_list: ", dataset[name][i][0].values()[0]
				r_list.append(dataset[name][i][0].values()[0])
...
k = KEGG()
data = dict()
list_path = open("hsa_gene_50_list.txt").read().replace('path:','').split('\n') # create list of hsa pathways
...
for hsa in list_path:
		...
		for r_id in r_ids: # handles entries with more than one reaction IDs
			...
			if r_id != '':
				if (len(list_check)==0) | (r_id not in list_check) :
					print("passed through the if statement for reaction ids")
	...
	for name in gene_names:
		if name in data: # if gene already exists, append additional reaction info
			...
			data[name] = vals
		print "current gene symbol and its information", data[name]

with open('KEGG_DB_50.json','w') as f: # save as .json
	json.dump(data, f)